...
namespace OrderStockManager.Services
{
    public class SalesViewExcelService : SupportExcel, ISalesViewExcelService, IDisposable
    {
        ...
        /// 年表示
        /// </summary>
        protected CellStyle year_title = new CellStyle(format: FORMAT.STRING, border: BORDER.BOX, alignment: HALIGNMENT.CENTER, color: COLOR.LIGHTGREEN, control: CONTROL.SHRINK, font: FONTSTYLE.PGOTHIC_11_BOLD);
        /// <summary>
        /// 月表示
        /// </summary>
        ...
        public byte[] CreateXlsxOneSheetBySalesView(CustomParameterModel param)
        {
            ...
            try
            {
                ...
                {
                    // 事務所データ
                    ICollection<OfficeModel> offices = dbContext.OfficeModels.Where(ofs => ofs.Deleted == false).Where(ofs => ofs.Code != ServiceResource.CompanyCode).ToList();
                    ...
                    {
                        group = dbContext.GroupModels.Where(gm => gm.Id == param.GroupId).SingleOrDefault();
                    }
                    if (group == null && param.MakerId.HasValue)
                    {
                        maker = dbContext.MakerModels.Where(mm => mm.Id == param.MakerId).SingleOrDefault();
                    }
                    else
                    {
                    ...
                    // データ抽出
                    List<SalesViewInterfaceModel> outputData;
                    RepositoryResult<IEnumerable<SalesViewInterfaceModel>> result;
                    result = service.GetSalesViewsForInterface(param, 18);
                    if (result.Code != System.Net.HttpStatusCode.OK)
                        return null;
                    outputData = result.resultData.ToList();
                    ...
                    // サインイン
                    int userId = AuthenticationManager.User.Identity.GetUserId<int>();
                    ...
                    */
                    ...
                    foreach (var productData in outputData)
                    {
                        ...
                        // 年度＋月の表示
                        check = startDate;
                        WriteCell(0, baseRowIndex, param.Year.ToString() + "年度", year_title);
                        ...
                        for (int i = 0; i < ONE_ROW.COLS_REPEAT_MONTH; i++)
                        {
                            SalesInterfaceModel work = productData.SalesList.Where(x => x.detail_date == check).SingleOrDefault();
                            if (work == null)
                            {
                ...
                using (DataContext dbContext = DataContext.Create())
                {
                    ...
                    foreach (var productData in outputData)
                    {
                        ...
                        for (int i = 0; i < MULTI_CONFIG.COLS_REPEAT_MONTH; i++)
                        {
                            ...
                            {
                                int j = 0;
                                SalesOfficeInterfaceModel officeWork;
                                foreach (var office in offices)
                                {
...
#endif
        public bool ReadXlsxToSalesView(FileInfo fileInfo)
        {
            try
            {
                int companyOfficeId = 0;
                if (OpenBook(fileInfo.OpenRead()) == false)
                {
                    return false;
                }
                ...
                {
                    // 全社用コードの取得
                    companyOfficeId = dbContext.OfficeModels.Where(o => o.Code == ServiceResource.CompanyCode).Select(o => o.Id).SingleOrDefault();
                    if (companyOfficeId == 0)
                    {
                        return false;
                    }
                    // シートの判定
                    if (GetSheetByIndex(0) == null)
                    {
                        return false;
                    }
                    ...
                    using (DbContextTransaction tx = dbContext.Database.BeginTransaction(System.Data.IsolationLevel.Serializable))
                    {
                        ...
                        while (true)
                        {
                            baseRow = ONE_ROW.ROW_START + index * ONE_ROW.ROWS_ONE_PRODUCT + 1; // 商品情報行を基準

                            // 商品コードを取得
                            if (!TryReadCell(0, baseRow, out productCode) || productCode != "商品コード")
                            {
                                if (index == 0)
                                {
                                    // 初回のみエラー
                            ...
                            if (!TryReadCell(1, baseRow , out productCode))
                            {
                                // 商品コードが存在しないと何も出来ない
                                tx.Rollback();
                                return false;
                            ...
                            {
                                // 処理対象年月を取得
                                if(!TryReadCell(ONE_ROW.COL_START_MONTH + colCount, baseRow, out targetDate))
                                {
                                    tx.Rollback();
                                    return false;
                                }
                                targetDate = targetDate.AddDays((targetDate.Day - 1) * -1);
                                if(minDate > targetDate)
                                {
                                    minDate = targetDate;
                                }
                                // 発注/入荷データの更新処理
                                TradeModel trade = dbContext.TradeModels.Where(tm => tm.ProductModelId == product.Id)
                                    .Where(tm => tm.TargetDate == targetDate).SingleOrDefault();
                                if (trade == null)
                                {
                                ...
                                if (saveChangeNew)
                                {
                                    dbContext.TradeModels.Add(trade);
                                    saveChangeNew = false;
                                }
                                dbContext.SaveChanges();

                                // 販売情報の更新処理
                                SalesModel sales = dbContext.SalesModels.Where(sm => sm.ProductModelId == product.Id)
                                    .Where(sm => sm.TargetDate == targetDate)
                                    .Where(sm => sm.OfficeModelId == companyOfficeId).SingleOrDefault();
                                if (sales == null)
                                {
                                    sales = new SalesModel();
                                    sales.ProductModelId = product.Id;
                                    sales.TargetDate = targetDate;
                                    sales.OfficeModelId = companyOfficeId;
                                    saveChangeNew = true;
                                }
                                else if (sales.Deleted == true)
                                {
                                    sales.Plan = 0;
                                    sales.Sales = 0;
                                    sales.Deleted = false;
                            ...
                            dbContext.Database.ExecuteSqlCommand(
                                ...
                                minDate, DateTime.Now.Date, product.Id);
